...
async function run() {
  ...
  log('corner back at top: ' + await js(win, `(() => {
    const btn = document.querySelector('[data-dsh-chat-corner]');
    if (!btn) return 'no-btn';
    const r = btn.getBoundingClientRect();
    const svg = btn.querySelector('svg')?.getBoundingClientRect();
    return 'top=' + Math.round(r.top) + ' btn=' + Math.round(r.width) + 'x' + Math.round(r.height) +
      ' icon=' + (svg ? Math.round(svg.width) + 'x' + Math.round(svg.height) : 'none');
  })()`))
  log('session log hidden: ' + await js(win, `(() => {
    const b = [...document.querySelectorAll('button')].find((x) => x.textContent.includes('Session log'));
    if (!b) return 'no-session-log';
    return getComputedStyle(b).display === 'none' ? 'hidden' : 'VISIBLE';
  })()`))
